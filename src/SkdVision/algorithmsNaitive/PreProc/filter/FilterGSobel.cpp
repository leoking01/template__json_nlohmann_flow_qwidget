#include "FilterGSobel.h"

//const  std::string  FilterGSobel::name   = "FilterGSobel";

//FilterGSobel::FilterGSobel(MODE_CV_HCON_TYPE   mode_t ):EnkAlgorithmBase(mode_t )
//{
//    mode_cv_or_hcon = mode_t ;
//    class_name = "FilterGSobel";
//}
#if   0     //  #endif     #if   0
FilterGSobel(MODE_CV_HCON_TYPE   mode_t ) ;
...
virtual double  loadData_hcon()    {         data_in_hcon.Image.ReadImage( "/home/alex/images/tongzhou_a/2.bmp"  );
                                            return  0 ;}
virtual double  run_hcon()    {         try {
       ...
       //            data_in_hcon.ImageOutput.WriteImage("png" ,255, "FilterGSobel.output.png") ;
                                       }  catch (HalconCpp::HException  ex) {
       std::cout<< "run, ex.ErrorCode() : "<<ex.ErrorCode()   <<std::endl;
   }
                                       return   0; }
virtual double  exportData_hcon()    {        try {
       if( data_out_hcon.Image.IsInitialized())
           data_out_hcon.Image.WriteImage("png" ,255, "FilterGSobel.hcon.output.png") ;
                                             }catch (HalconCpp::HException  ex) {
