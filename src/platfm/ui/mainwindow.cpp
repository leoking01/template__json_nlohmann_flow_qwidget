...
void MainWindow::addToolProc_from_ui() {
  ...
  res_insert =   this->mp_flowApp->addToolItem_from_ui(nums, toolItemName, timeIdStr, soleName, m_flow_data);
    if (res_insert) {
        //添加到流程表
        if( 0 )
...
void MainWindow::loadFlowData() {
    try {
        ...
        //this->mp_flowApp->m
        //update app
        bool res = this->mp_flowApp->from_json(this->m_json,m_flow_data);
        if (!res) {
...
//        return ;
        this->mp_flowApp->to_json(this->m_json,m_flow_data);//return ;
